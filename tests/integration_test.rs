...
fn create_explorer() {
    ...
    // 1. Create Explorer
    let mut explorer = Explorer::new(266, tx_eto, rx_ote, rx_pte, Duration::from_millis(500));
    ...
    let expected: ExplorerToOrchestrator<ExplorerBagContent> =
        ExplorerToOrchestrator::StartExplorerAIResult { explorer_id: 266 };
    ...
    let response = rx_eto.recv().expect("Error while receiving from Explorer");
    let expected: ExplorerToOrchestrator<ExplorerBagContent> =
        ExplorerToOrchestrator::KillExplorerResult { explorer_id: 266 };
    assert_eq!(format!("{response:?}"), format!("{expected:?}"));

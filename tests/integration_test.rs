...
mod manual_mode {
    ...
    fn test_move_to_planet() {
        ...
        let expected1: ExplorerToOrchestrator<ExplorerBagContent> = ExplorerToOrchestrator::NeighborsRequest {explorer_id : EXPLORER_ID, current_planet_id: PLANET_ID};
        let expected2: ExplorerToPlanet = ExplorerToPlanet::SupportedResourceRequest {explorer_id : EXPLORER_ID};
        let expected3: ExplorerToPlanet = ExplorerToPlanet::SupportedCombinationRequest {explorer_id : EXPLORER_ID};
...
mod ai_mode {
    ...
    fn test_generate_intentions() {
        ...
        tx_pte.send(PlanetToExplorer::SupportedResourceResponse {resource_list}).expect("Error while sending to the Explorer");

        // 9. Wait for Explorer Intention. It should be a Generate(Carbon), so it should ask the Planet to generate carbon
        let expected = ExplorerToPlanet::GenerateResourceRequest {explorer_id : EXPLORER_ID, resource : BasicResourceType::Carbon};
    ...
    fn test_move_intentions() {
        ...
        tx_ote.send(OrchestratorToExplorer::NeighborsResponse {neighbors}).expect("Error while sending to the Explorer");

        // 9. Wait for Explorer Intention. It should be a Move(PLANET_2_ID), so it should ask the Orchestrator to move
        let expected : ExplorerToOrchestrator<ExplorerBagContent> = ExplorerToOrchestrator::TravelToPlanetRequest {explorer_id : EXPLORER_ID, current_planet_id : PLANET_ID, dst_planet_id : PLANET_2_ID};
